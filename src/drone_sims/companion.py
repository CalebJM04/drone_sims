...
def global_to_local_ned(
    ...
    *,
    ...
    """Convert WGS84 coordinates to a local tangent-plane approximation.

    This equirectangular projection is intentionally limited to a small mission
    area. All aircraft must use the same configured reference point.
    """
...
class CompanionService:
    """Hardware-independent collision service core for one aircraft.

    Scheduling uses monotonic time while packet timestamps use synchronized
    mission time. Stale PX4 state suppresses all commands; PX4 must then execute
    its configured offboard-loss action.
    """
    ...
    def __init__(
        ...
        # Startup without a first PX4 state is "not ready", not a state-loss
        # event. Count only fresh-to-stale transitions after initialization.
    ...
    def step(self, monotonic_s: float, mission_time_s: float) -> None:
        ...
        for payload in self.radio.receive():
            self._ingest(payload, monotonic_s)
        ...
        if (
            ...
            # A zero-velocity nominal stream satisfies PX4's Offboard proof-of-
            # life requirement and produces a hover/stop request if the pilot
            # deliberately changes into Offboard mode.
    ...
    def _ingest(self, payload: bytes, received_at: float) -> None:
        try:
