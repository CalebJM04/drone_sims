...
<section class="grid two">
  ...
  <article class="card"><h2>Companion deployment</h2><p class="note">Raspberry Pi receives PX4 global telemetry, converts it to the shared mission frame, exchanges LoRa state, and emits bounded local-NED velocity setpoints. PX4 remains responsible for stabilization and failsafes.</p></article>
</section>
...
kpis();gates();scenarioChart();networkPlot();tracking();traces();
</script></body></html>'''
