...
|---|---|---|
| Collision math | Geometry tests, sampling oracle, 10k noisy trials | Logged GNSS versus independent truth |
| Frozen protocol | Golden bytes, 10k corruptions, CRC, malformed input | Radio/UART electrical integration |
| Restart/replay | Boot ID, rollover, delayed retired-session rejection | Repeated real power-cycle tests |
| Neighbor state | Reorder, capacity, expiry, eviction, 6 s uncertain propagation | Final BRAM/timing implementation |
...
| FPGA | Parser/table integration, 5k predictor vectors, Yosys elaboration | Timing closure, power, pins, board I/O |
| Endurance | 300 simulated seconds, 12 nodes, relay routing | Thermal/power/soak testing |
| Acceptance | Frozen JSON thresholds, 4 m operational margin and 14-gate readiness report | Hardware gate extension |
...
a point-mass velocity model except for the PX4 SIH runs. Yosys elaboration is not
device-specific place-and-route or timing closure. The final readiness report
therefore lists RF, GNSS, FPGA-board, electrical, airframe and flight tests as
explicit hardware-only work.
